import React, { Component } from 'react'


import { SearchBar, Panel, PanelHeader, PanelBody, PanelFooter } from 'react-weui';
import { Cells, CellsTitle, Cell, CellFooter, CellBody, Badge } from 'react-weui';

import style from './index.less'

// import iconSrc from './1.png';

// const appMsgIcon = <img src="data:image/png;base64," />

let datas = []

class Sudoku extends Component {
    constructor(props) {
        super(props)
        console.log(props)
        this.state = {
            searchText: '',
            results: [],
            cnt: 0
        };
    }
    handleChange(text) {
        console.log(text)
        datas.push(text)
        this.setState({
            results: datas,
            searchText: text,
            cnt: datas.length
        });
    }

    render() {
        return (
            <div>
            <SearchBar onChange={this.handleChange.bind(this)} placeholder="Search" lang={{
                cancel: 'Cancel'
            }}/>

              <Panel className={style.search} style={{
                display: this.state.searchText ? null : 'none',
                marginTop: 0
            }}>
                    <PanelHeader>
                        results: <Badge preset="body">{this.state.cnt}</Badge>
                    </PanelHeader>
                    <PanelBody>
                    <CellsTitle>List with link</CellsTitle>
                    <Cells>
                        {
            this.state.results.length > 0 ?
                this.state.results.map((item, i) => {
                    return (
                        <Cell key={i} access>
                            <CellBody>
                                {item}
                            </CellBody>
                            <CellFooter/>
                        </Cell>
                    )
                }) : <div>Can't find any！</div>}
                    </Cells>
                    </PanelBody>
                    <PanelFooter href="javascript:void(0);">
                    </PanelFooter>
                </Panel>
            </div>
        )
    }
}

Sudoku.propTypes = {

}

Sudoku.defaultProps = {

}

export default Sudoku